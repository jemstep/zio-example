...
class ExampleUnitSpec extends Specification {
  ...
  trait TestCups extends Cups {
    override val cups: Cups.Service[Cups] = new Cups.Service[Cups] {
  ...
  object TestEnvironmentWithDummyLogging extends Console with Cups
      with TestConsole with TestCups

  object TestEnvironmentWithRealLogging extends Console with Cups
      with TestCups with Console.Live
  ...
  "cups example" should {
    ...
    "produce the expected results without actual logging using the test environment with a dummy console" in {
      runtime.unsafeRun(ZIOApp.cupsExample.provide(TestEnvironmentWithDummyLogging)) must_==
    ...
    "produce the expected results with actual logging using the test environment with real logging" in {
      runtime.unsafeRun(ZIOApp.cupsExample.provide(TestEnvironmentWithRealLogging)) must_==
      (("Right(A successfuly parsed body)",
        "Left(An unsuccessfuly parsed body)",
        "Right(Another successfuly parsed body)"))
    }
